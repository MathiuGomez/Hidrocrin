    ...
    <div className="w-full max-w-7xl mx-auto px-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
        ...
        <div className="h-full">
          <div 
            className="relative overflow-hidden rounded-3xl border border-emerald-100/50 shadow-xl hover:shadow-2xl transition-all duration-500 bg-gradient-to-br from-white via-emerald-50/30 to-white h-full"
          >
            {/* Decoración de fondo */}
            ...
            <div className="relative p-8 flex flex-col h-full">
              <div className="flex items-center gap-4 mb-8">
                <div className="relative">
                  <div className="absolute inset-0 bg-emerald-400 rounded-2xl blur-lg opacity-30 animate-pulse" />
                  <div className="relative h-14 w-14 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl flex items-center justify-center shadow-lg">
                    <CalendarIcon className="h-7 w-7 text-white" />
                  </div>
                </div>
                <div>
                  <h3 className="text-3xl font-bold bg-gradient-to-r from-emerald-700 to-emerald-500 bg-clip-text text-transparent">
                    Calendario de Actividades
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">Planifica y organiza tus eventos</p>
                </div>
              </div>
              ...
              <div className="flex justify-center flex-1 flex-col">
                ...
                <div className="mt-6 flex items-center justify-center gap-2 text-sm text-gray-600 bg-emerald-50/50 rounded-xl p-3">
                  <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span>Las fechas <span className="font-bold text-emerald-600 underline">subrayadas</span> tienen actividades</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="h-full">
          <div 
            className="relative overflow-hidden rounded-3xl border border-emerald-100/50 shadow-xl hover:shadow-2xl transition-all duration-500 bg-gradient-to-br from-white via-teal-50/20 to-white h-full"
          >
            {/* Decoración de fondo */}
            ...
            <div className="relative p-8 flex flex-col h-full">
              <div className="mb-6">
                <h3 className="text-2xl font-bold bg-gradient-to-r from-teal-700 to-emerald-600 bg-clip-text text-transparent mb-2">
              ...
              {(selectedActivities.length > 0 ? selectedActivities : upcomingActivities).length > 0 ? (
                <div className="space-y-4 flex-1">
                  {(selectedActivities.length > 0 ? selectedActivities : upcomingActivities).map((activity, index) => (
                    <div
                      key={activity.id}
                      className="group relative overflow-hidden rounded-2xl bg-white border border-emerald-100 hover:border-emerald-300 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                    >
                      {/* Barra lateral de color */}
                      <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-gradient-to-b from-emerald-500 to-teal-500 group-hover:w-2 transition-all duration-300" />
                      
                      {/* Badge de número */}
                      <div className="absolute top-4 right-4 h-8 w-8 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">
                        {index + 1}
                      </div>

                      <div className="p-5 pl-6">
                        <div className="flex items-start gap-3 mb-3">
                          <div className="mt-1 h-10 w-10 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-110 transition-transform duration-300">
                            <CalendarIcon className="h-5 w-5 text-white" />
                          </div>
                          <div className="flex-1 min-w-0 pr-8">
                            <h4 className="font-bold text-gray-800 text-base mb-2 leading-tight group-hover:text-emerald-700 transition-colors">
                              {activity.title}
                            </h4>
                            <p className="text-sm text-emerald-600 font-semibold bg-emerald-50 inline-block px-3 py-1 rounded-full">
                              {format(activity.date, "d 'de' MMMM", { locale: es })}
                            </p>
                          </div>
                        </div>

                        <p className="text-sm text-gray-600 mb-4 leading-relaxed pl-13">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
                  <div className="h-20 w-20 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center mb-4">
